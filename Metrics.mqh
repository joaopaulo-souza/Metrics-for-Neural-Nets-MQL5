...
public:
  ...
  // is the most likely class
   ...
   //Mean Squared Error
   double MSE(matrix &R, matrix &Y);

   //Root Mean Squared Error
   double RMSE(matrix &R, matrix &Y);

   //Mean Absolute Error
   double MAE(matrix &R, matrix &Y);
...
double Metrics::MSE(matrix &R, matrix &Y)
{
matrix Err;
Err = R - Y; 
Err = MathPow(Err,2);
...
double Metrics::RMSE(matrix &R, matrix &Y)
{
matrix Err;
Err = R - Y; 
Err = MathPow(Err,2);
...
double Metrics::MAE(matrix &R, matrix &Y)
{
matrix Err;
Err = R - Y; 
Err = MathAbs(Err);
